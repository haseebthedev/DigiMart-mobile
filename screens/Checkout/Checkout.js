...
const Cart = ({ route, navigation }) => {
  ...
  const getUserInfo = async () => {
    await api
      ...
      .then((res) => {
        const { email, phoneNumber, address, name, city } =
      ...
      await api
        .post(
          '/buyer/product/order',
          {
            ...
            deliveryDate: '2021-12-10T07:03:11.212+00:00',
  ...
  const fetchPaymentSheetParams = async () => {
    const response = await api.post(
      `/buyer/stripe-checkout-mobile`,
      {
        amountToCharge: Math.floor(totalPrice) * 100
      },
      {
        headers: { Authorization: `Bearer ${user.token}` }
      }
    ...
    let paymentIntent = response.data.data.paymentIntent;
    let ephemeralKey = response.data.data.ephemeralKey;
  ...
  const initializePaymentSheet = async () => {
    ...
    const { error } = await initPaymentSheet({
      ...
      merchantDisplayName: 'STRIPE MOBILE'
    });
    if (!error) {
      setLoading(true);
    }
      ...
      <StripeProvider publishableKey={STRIPE_KEY}>
        ...
        >
          ...
          <View style={{ marginBottom: 20 }}>
            ...
            >
              <View style={{ flexDirection: 'row' }}>
                <Image source={paymentIcon} style={{ width: 20, height: 20 }} />
                <Text style={{ fontFamily: FONTS.PoppinsBold, marginLeft: 10 }}>
                  Payment Option
                </Text>
              </View>
        ...
        >
          ...
          <View style={styles.modal}>
            ...
            >
              Your order has been placed. Please check email for further
              details. Your Order Tracking Id is #{' '}
              <Text
                style={{
                  fontFamily: FONTS.PoppinsBold,
                  fontSize: FONTS.subhead4,
